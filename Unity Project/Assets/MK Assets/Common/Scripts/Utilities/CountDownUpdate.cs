...
{ // NOTE: Performance wise, it is slow, but result wise might be better than the IEnumerator solution.
    ...
    { // Remember: MonoBehaviour is just to use its Update function, but still it should be on attached on some GameObject
        ...
        public void StartCounting(double _count, double _limit, bool _increasing, Text _text, Action<bool, string> _onDone)
        {
            display = _text;
            StartCounting(_count, _limit, _increasing, _onDone);
        }

        public void StartCounting(double _count, double _limit, bool _increasing, TMP_Text _tmpText, Action<bool, string> _onDone)
        {
            displayTMP = _tmpText;
            StartCounting(_count, _limit, _increasing, _onDone);
        }
        ...
        {
            StopCounting(); // stop previous one first, just in case
            count = _count;
            limit = _limit;
